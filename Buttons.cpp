...
Buttons::Buttons(QWidget *parent, int window_x, int window_y) {
    ...
    QHBoxLayout *layout_manager = new QHBoxLayout(this);
    setLayout(layout_manager);

    //constructor
    button_struct_list.append(button_details_strc("tan", "black", true, 50, 100, 50, 100));
}
...
//    this seems useless so im commenting it out
    QLayoutItem *item;
    while (layout() && (item = layout()->takeAt(0)) != nullptr) {
        delete item->widget();
        delete item;
    }
